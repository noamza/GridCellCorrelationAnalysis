    ...
    if ~isempty(good) && length(good) >= 2        
        ...
        for c1i = 1:length(good)-1
            for c2i = c1i+1:length(good)
                ...
                imagesc(az, cc); xlabel(az, sprintf('c%d x c%d',c1.ind,c2.ind));colormap(az,'jet');
                plot(az, size(cc,2)/2,size(cc,1)/2,'md','MarkerFaceColor','w','MarkerSize',7);
                %intersection union
                plot(az, c1.module.x, c1.module.y,'w'); plot(az, c2.module.x, c2.module.y,'r');  
                title(az, sprintf('Int/Union=%.2f',Intersection_Over_Union(...
