%lag in ms
function [ pcor ] = timeCorrelationSmoothed( train1,train2,p)
    lag = round(p.lagms);
    lowf = p.sigma;
    win = p.hamwin;
    ...
    if p.hamwin>0
        ...
        %win = win/sum(win);
        train1smooth = movmean(train1,p.hamwin);
        train2smooth = movmean(train2,p.hamwin);
        %
        toc
    end
    if p.sigma>0
   ...
   %pcor = xcorr(train1Ham-mean(train1Ham), train2Ham-mean(train2Ham),lag,'coef');%SAME AS NEXT LINE
    pcor = xcov(train1smooth, train2smooth,lag,'coef'); % TRY AS MATRIX 'coef'
