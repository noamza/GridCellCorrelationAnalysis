...
load(sprintf('.\\data\\1kSpaceShuffleNBins%d',nb));
...
%% COMPARE TO SHUFFLING
    ...
    n = len(ptcb); show = [1,2,3,4,5,6];
    pp = ptsbma<= n*0.01;  sum(pp(:,show))/length(pairs) %LARGEST VAL (FIRST DESC)
    pp = n*0.99 <= ptsbma; sum(pp(:,show))/length(pairs) %SMALLEST VAL (LAST DESC)
