...
function group_Callback(hObject, eventdata, handles)
    ...
    l = length(handles.model.groups.(handles.model.k{gid}));
    set(handles.status, 'String', sprintf('group size: %d',l));
    guidata(hObject,handles)
    g = handles.model.groups.(handles.model.k{handles.model.gid});
...
function group_CreateFcn(hObject, eventdata, handles)
    ...
    handles.model.k = k;
    set(hObject,'String',cellstr(num2str((1:length(k))')));
    if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
        set(hObject,'BackgroundColor','white');
    end
    handles.model.gid = 1;
